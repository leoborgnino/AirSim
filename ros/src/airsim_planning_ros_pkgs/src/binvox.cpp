...
leatherman::VoxelGrid& leatherman::Grid()
{
...
bool leatherman::convertBinvoxToOctomapMsg(std::string filename, double resolution, octomap_msgs::Octomap &msg)
{
  ...
  //octree->insertRay(*point_cloud, octomap::point3d(0.0f, 0.0f, 0.0f)); // FIXME
...
bool leatherman::convertBinvoxToVector3d(std::string filename, std::vector<Eigen::Vector3f> &voxels)
{
...
bool leatherman::voxelizeMesh(std::string filename, double resolution, std::vector<Eigen::Vector3f> &voxels)
{
...
void leatherman::getOccupiedVoxelsInOcTree(octomap::OcTree* octree, std::vector<Eigen::Vector3f> &voxels)
{
  Eigen::Vector3f v;
  for (octomap::OcTree::iterator it = octree->begin(octree->getTreeDepth()), end = octree->end(); it != end; ++it)
  {
...
// void leatherman::getOccupiedVoxelsInCollisionMap(const arm_navigation_msgs::CollisionMap &map, std::vector<Eigen::Vector3f> &voxels)
// {
...
bool leatherman::getOccupiedVoxelsInBinvoxFile(std::string binvox_filename, std::vector<Eigen::Vector3f> &voxels)
{
  ...
  *input >> version;
  ...
  double res = (1.0/double(scale))/double(maxSide);
  ROS_INFO("size: %d  width: %d  depth: %d  height: %d  maxSide: %d   res:  %f", size, width, depth, height, maxSide, res);
  ...
  *input >> value;    // read the linefeed char
  ...
  while((end_index < size) && input->good()) 
  {
    *input >> value >> count;    
	...
	bool value_bool = int(value);
	//if (value_bool == true)
	//  std::cout << int(value_bool) << std::endl;
        tree->updateNode(endpoint, value_bool, false);
	//if (value)
	//  std::cout << int(value) << std::endl;
